...
class simple_attn(nn.Module):
    ...
    def forward(self, x, name='0'):
        ...
        qkv = self.qkv_proj(x).permute(0, 2, 3, 4, 1).reshape(B, D*H*W,
                                                              self.heads,
                                                              3*self.headc)
...
class SRNO3D(nn.Module):
    ...
    def query_rgb(self, coord, cell):
        ...
        for vz in vz_lst:
            for vx in vx_lst:
                for vy in vy_lst:
                    ...
                    old_coord = F.grid_sample(pos_lr,
                                              coord_.unsqueeze(0).flip(-1),
                                              mode='nearest',
                                              align_corners=False)
        ...
        grid = torch.cat([*rel_coords, *feat_s,
                          ...
                          .repeat(1, 1, coord.shape[0],
                                  coord.shape[1], coord.shape[2])], dim=1)
        x = self.conv00(grid)
        x = self.conv0(x, 0)
        x = self.conv1(x, 1)
        ...
        ret = ret + F.grid_sample(self.inp, coord.unsqueeze(0).flip(-1),
...
def Meshgrid(z, x, y):
    grid = torch.meshgrid(
        torch.arange(z), torch.arange(x),
        torch.arange(y), indexing='ij')
    stack = torch.stack(grid, dim=-1)
    return stack
# -------------------------SIREN model------------------------- #


class NeRF(nn.Module):
    def __init__(self, num, W, num_encoding_functions, skips):
        super(NeRF, self).__init__()
        self.W = W
        self.skips = skips
        self.pts_linears = nn.ModuleList([nn.Linear(num_encoding_functions*2*3
                                         + 3, W)] + [nn.Linear(W, W) if i
                                         not in skips else nn.Linear
                                         (W + num_encoding_functions*2*3 +
                                          3, W) for i in range(num-1)])
...
class Composite(nn.Module):
    def __init__(self, model2, model3):
    ...
    def forward(self, x):
        latent = x
        coord = Meshgrid(latent.shape[2], latent.shape[3], latent.shape[4])
        coord = coord.reshape(-1, 3).to(torch.float32)
        latent1 = self.model2(coord)
        latent1 = latent1.reshape(1, 1, latent.shape[2], latent.shape[3],
                                  latent.shape[4])
        y = self.model3(latent1)
        return y


def create_json_file(data):
    json_data = json.dumps(data, indent=4)
    with open("data_edsr.json", "a") as json_file:
